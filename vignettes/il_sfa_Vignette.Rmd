---
title: "True Fixed Effect Stochastic Frontier Models estimation via Integrated Likelihood: il.sfa package"
...
a bunch of background function (most of them written in `Rcpp`) used to make the code computationally 
...
The il_sfa function allows to estimate the frontier model defined in Belotti and Ilardi (2018) and 
Bellio and Grassetti (2023) using a feasible, generalizable and computationally efficient approach 
...
To use the estimation function one pass a set of objects including all the data used in model specification. 
...
- a vector for the variable influencing the variance of efficiency-specific error term.
...
- `het` to specify the presence of heteroschedasticity affecting the efficiency related 
...
fails. The fifth generated dataset presents this kind of issue.
...
In case of `HN` model estimation, the heteroschedastic efficiency related errors 
are still not supported. The estimation routine again considers some preliminary checks 
and operate a feasible estimation giving a warning message. Here the problem is also connected
to the wrong model specification. Data are generated considering the exponential 
efficiency related error specification.
...
## Developing a simulation study using il.sfa and DGP functions
...
The example considers 1000 replications to show that the code is quite efficient from 
a computational point of view. Changing the nsim parameter, one can consider different 
...
The estimation results are quite similar but the IL MSE are alway a bit lower than the PDE ones.
